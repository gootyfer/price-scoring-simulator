...
function render() {
  ...
  elements.table.innerHTML = evaluated.map(result => {
    ...
    return result.name === winner.name ? `${row}<tr class="winner-explanation"><td colspan="7">${formula.explain}</td></tr>` : row;
  }).join("");

  elements.cards.innerHTML = evaluated.map(result => {
    const [statusText, statusClass] = status(result);
    const cardClass = result.excluded ? "excluded" : result.name === winner.name ? "winner" : "";
    const explanation = result.name === winner.name ? `<p class="winner-explanation" role="note">${formula.explain}</p>` : "";
    return `<article class="offer-card ${cardClass}"><header><h3>${result.name}</h3><span class="status ${statusClass}">${statusText}</span></header><dl><div><dt>Oferta</dt><dd>${money.format(result.offer)}</dd></div><div><dt>Baja</dt><dd>${percentage.format(reduction(result))} %</dd></div><div><dt>Técnica</dt><dd>${decimal.format(result.technical)} pts</dd></div><div><dt>Precio</dt><dd>${pointText(result.price)}</dd></div><div><dt>Total</dt><dd class="total-mobile">${pointText(result.total)}</dd></div></dl>${explanation}</article>`;
  }).join("");
}
